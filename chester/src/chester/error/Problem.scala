...
import chester.i18n.*
import chester.ToDoc
import chester.DocConf
import chester.Doc
import chester.FileContent
...
private def renderFullDescription(desc: FullDescription)(using options: DocConf, sourceReader: SourceReader): Doc = {
  ...
  val explanationsDoc = desc.explanations.map { elem =>
    ...
    elem.span0.flatMap(sourceReader.apply) match {
      case Some(lines) =>
        val sourceLines = lines.map((lineNumber, line) => dt"${Doc.text(lineNumber.toString)}" <+> Doc.text(line, Styling.BoldOn))
