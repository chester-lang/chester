...
let load path : t =
  ...
  let rec parse_lines lines current_pkg pkgs =
    ...
    | line :: rest ->
        ...
        else parse_lines rest current_pkg pkgs
  in
  let pkgs = parse_lines lines None [] in
  let pkgs =
    List.map
      (fun p -> { p with functions = List.rev p.functions })
      (List.rev pkgs)
  in
...
let summary t =
  t
  |> List.map (fun pkg ->
      Printf.sprintf "%s (%d functions)" pkg.name (List.length pkg.functions))
  |> String.concat ", "
...
let has_function t pkg fn =
  List.exists (fun p -> p.name = pkg && List.exists (( = ) fn) p.functions) t
