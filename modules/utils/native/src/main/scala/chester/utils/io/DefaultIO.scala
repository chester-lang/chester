...
object DefaultPathOps extends PathOps[_root_.os.Path] {
  override inline def of(path: String): _root_.os.Path = os.Path(path)

  override inline def join(p1: os.Path, p2: String): _root_.os.Path =
    (p1 / os.SubPath(p2)).asInstanceOf[os.Path]

  override inline def asString(p: os.Path): String = p.toString

  override inline def baseName(p: os.Path): String = p.last

  override inline def isAbsolute(p: os.Path): Boolean = p.toNIO.isAbsolute

  override inline def isRelative(p: os.Path): Boolean = !p.toNIO.isAbsolute
}

given DefaultIO: IO[Id] {
  type Path = os.Path

  override inline def pathOps: PathOps[Path] = DefaultPathOps
  ...
  override inline def removeWhenExists(path: Path): Boolean =
    os.remove(path.resolveFrom(workingDir), checkExists = true)

  override inline def getHomeDir: Path = os.Path(
  ...
  override inline def downloadToFile(url: String, path: Path): Unit = {
    val target = path.resolveFrom(workingDir)
    val connection = URL(url)
    val in = BufferedInputStream(connection.openStream())
    val out = Files.newOutputStream(target.toNIO)
    try {
