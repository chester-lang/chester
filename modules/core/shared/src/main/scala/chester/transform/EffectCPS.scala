...
/** A small, type-directed CPS transformation scaffold for user-defined effects.
  *
  * The current elaborator tracks effect rows in `AST.Pi` types. This pass rewrites effectful function
  * types into continuation-passing style and offers a minimal expression transformation that threads a
  * continuation through lambdas. It is designed as a future-facing building block: the pass is not
  * wired into the pipeline yet, but gives us a single place to evolve CPS lowering for user-defined
  * effects (and optionally IO) without disturbing type checking.
  *
  * Design notes:
  *   - Only `Pi` types with non-empty effect rows (or io when `transformIO` is enabled) are rewritten.
  *   - Effectful `Pi` becomes a pure `Pi` that accepts an extra continuation parameter `k` of type
  *     `(resultTy) -> answerType` and returns `answerType`.
  *   - For expressions, lambdas are rewritten to accept the extra continuation and immediately call it
  *     on the transformed body. Other nodes are transformed structurally; application rewriting would
  *     require type information for the callee, which we don't have yet, so it is left untouched.
  */
object EffectCPS:
  ...
  /** Transform an expression with its (already known) type.
    *
    * The transformer is intentionally conservative: only lambdas gain explicit continuations. Call sites
    * are left as-is because we currently lack the callee's type information at this stage; wiring this
    * pass after elaboration with a typed AST will let us close that gap later.
    */
