...
object ElabContext:
  val defaultBuiltins: Set[String] = Set(
    "Integer", "Int", "String", "Bool", "Nat", "Natural",
    "Type", "U", "Universe",
    "true", "false"
  )
...
object Elaborator:
  /** Elaborate a CST into an AST with type inference
    * 
    * @param cst The concrete syntax tree to elaborate
    * @param reporter The reporter for errors (defaults to VectorReporter)
    * @param ctx The elaboration context with bindings (if None, creates default with reporter)
    * @param module The solver module to use (defaults to ProceduralSolverModule)
    * @return The elaborated AST and its inferred type
    */
