...
class Memoiser[T, U](tipe : MemoiserType) {
     ...
     */
    private[this] val map : java.util.Map[T, U] =
        (tipe match {
            case ValueKeys =>
                new java.util.concurrent.ConcurrentHashMap[T,U]()
            case IdentityKeys =>
                java.util.Collections.synchronizedMap(new java.util.WeakHashMap[T,U]())
        })
