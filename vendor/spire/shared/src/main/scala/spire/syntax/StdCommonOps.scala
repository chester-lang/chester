...
final class SeqOps[@sp A, CC[A] <: Iterable[A]](as: CC[A]) { // fixme
  ...
  protected def fromArray(arr: Array[A])(implicit cbf: Factory[A, CC[A]]): CC[A] = {
  ...
  protected def fromSizeAndArray(size: Int, arr: Array[A])(implicit cbf: Factory[A, CC[A]]): CC[A] = {
    val b = cbf.newBuilder
    b.sizeHint(size)
