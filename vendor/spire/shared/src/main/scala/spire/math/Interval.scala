...
sealed abstract class Interval[A] extends Serializable { lhs =>
  ...
  @inline final protected def isClosed(flags: Int): Boolean = flags == 0
  @inline final protected def isClosedLower(flags: Int): Boolean = (flags & 1) == 0
  @inline final protected def isClosedUpper(flags: Int): Boolean = (flags & 2) == 0

  @inline final protected def isOpen(flags: Int): Boolean = flags == 3
  @inline final protected def isOpenLower(flags: Int): Boolean = (flags & 1) == 1
  @inline final protected def isOpenUpper(flags: Int): Boolean = (flags & 2) == 2

  @inline final protected def lowerFlag(flags: Int): Int = flags & 1
  @inline final protected def upperFlag(flags: Int): Int = flags & 2

  @inline final protected def reverseLowerFlag(flags: Int): Int = flags ^ 1
  @inline final protected def reverseUpperFlag(flags: Int): Int = flags ^ 2
  @inline final protected def reverseFlags(flags: Int): Int = flags ^ 3

  final protected def lowerFlagToUpper(flags: Int): Int = (flags & 1) << 1
  final protected def upperFlagToLower(flags: Int): Int = (flags & 2) >>> 1

  @inline final protected def swapFlags(flags: Int): Int =
    ((flags & 1) << 1) | ((flags & 2) >>> 1)

  protected def lowerPairBelow(lower1: A, flags1: Int, lower2: A, flags2: Int)(implicit o: Order[A]): Boolean =
    lower1 < lower2 || lower1 === lower2 && (isClosedLower(flags1) || isOpenLower(flags2))

  protected def upperPairAbove(upper1: A, flags1: Int, upper2: A, flags2: Int)(implicit o: Order[A]): Boolean =
  ...
  // step.
  private def getStart(bound: Bound[A], step: A, unboundError: String)(implicit ev: AdditiveMonoid[A]): A =
    bound match {
