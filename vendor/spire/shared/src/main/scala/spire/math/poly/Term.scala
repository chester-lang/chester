...
object Term {
  ...
  private val superscript: (Char => Char) = Map(digitToSuperscript.toIndexedSeq*)

  private val removeSuperscript: (Char => Char) = Map(digitToSuperscript.map(_.swap).toIndexedSeq*)
}
